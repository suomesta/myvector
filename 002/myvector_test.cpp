...
const lest::test specification[] =
{
    ...
    CASE("002: public member types, and sizeof")
    {
        // check public member types for T is char, int, and double
        {
            ...
            EXPECT((std::is_same<char*, vec::iterator>::value));
            EXPECT((std::is_same<const char*, vec::const_iterator>::value));
            EXPECT((std::is_same<std::reverse_iterator<char*>, vec::reverse_iterator>::value));
            EXPECT((std::is_same<std::reverse_iterator<const char*>, vec::const_reverse_iterator>::value));
        }
        {
            ...
            EXPECT((std::is_same<int*, vec::iterator>::value));
            EXPECT((std::is_same<const int*, vec::const_iterator>::value));
            EXPECT((std::is_same<std::reverse_iterator<int*>, vec::reverse_iterator>::value));
            EXPECT((std::is_same<std::reverse_iterator<const int*>, vec::const_reverse_iterator>::value));
        }
        {
            ...
            EXPECT((std::is_same<double*, vec::iterator>::value));
            EXPECT((std::is_same<const double*, vec::const_iterator>::value));
            EXPECT((std::is_same<std::reverse_iterator<double*>, vec::reverse_iterator>::value));
            EXPECT((std::is_same<std::reverse_iterator<const double*>, vec::const_reverse_iterator>::value));
        }

        // check the result of sizeof(). Only minimum check is done
        {
            size_t minimum = sizeof(void*) + sizeof(size_t) + sizeof(size_t);
            EXPECT(minimum <= sizeof(myvector<char>));
            EXPECT(minimum <= sizeof(myvector<int>));
            EXPECT(minimum <= sizeof(myvector<double>));
        }
    },
